...
def build_metric_smoke_summary(
    ...
    for _, d5_row in d5.iterrows():
        ...
        for col, value in keys.items():
            if col in d1_rows.columns:
                d1_rows = d1_rows[d1_rows[col] == value]
        d3_depth1 = _select_row(d3[d3["prefix_depth"] == 1], keys)
        d4_buckets = _d4_bucket_map(d4, keys)
        rows.append(
            {
                **keys,
                "d1_level_count": int(len(d1_rows)),
                "d1_unique_codes": ";".join(str(int(value)) for value in d1_rows["unique_codes"]),
                "d1_entropy": ";".join(f"{float(value):.6f}" for value in d1_rows["entropy"]),
