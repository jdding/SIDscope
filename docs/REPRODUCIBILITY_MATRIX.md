...
| Adapter C0-C5 evidence | `docs/reproducibility/sidscope_source_license_config_inventory.csv`; `docs/reproducibility/conformance/*`; `examples/conformance_failure_fixture/*` | `python3 tools/verify_adapter_conformance_assets.py` | eight verified C0-C5 route reports, exact C1 failure fixture, and inventory evidence boundary | CPU seconds | fully runnable from release repo over compact evidence; source-dependent recomputation requires separately obtained upstream inputs |
...
| D7 labeled-trace release | `docs/reproducibility/d7_labeled_trace_rows.csv.gz`; `docs/reproducibility/d7_labeled_trace_release.json` | `python3 tools/verify_sidscope_d7_labeled_trace_release.py` | 125,000 deidentified beam rows, 2,500 traces, five cases, and verified label distributions | CPU seconds | fully runnable from release repo; user/item identities, raw SID paths, scores, and checkpoints are omitted |
...
| DIGER RQ-VAE adapter rows | DIGER Hugging Face Beauty or Yelp embeddings, `emb_map`, train/valid/test JSONL, and RQ-VAE checkpoint; compact C0--C5 reports | `python3 tools/run_v1_gate17_diger_intake.py --dataset-profile <beauty\|yelp> ...` | `sid_assignments.parquet`, `item_metadata.parquet`, `interactions.parquet`, preflight JSON, bounded D1-D5 smoke | CPU minutes after staging pinned upstream HF files | adapter released; Beauty and Yelp routes validated; upstream HF files/checkpoint required; no-license-detected caveat |
