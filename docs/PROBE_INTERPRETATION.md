...
| D2 aliasing | Multiple items share a full SID, or a prefix contains excessive mass | Shared leaf addresses or concentrated prefix load; D2 counts these as collision groups/items | Inspect alias groups by popularity and co-occurrence; require full-code uniqueness when the decoder needs one address per item. |
...
| D7 trace accounting | Unresolved paths, duplicate items or paths, ambiguous resolution, stale items, or target loss inside the recorded beam | Decoder-output observability or accounting failure; an ambiguous path is a valid full SID resolving to multiple items | First distinguish constrained from unconstrained decoding, then inspect generated path, resolved item, rank, score, target, and mapping revision. In trained constrained beams, compare target-code survival with unique target-item resolution. |
